...
export async function POST(request: NextRequest) {
  try {
    ...
    // Extract bid ID from pathname for authorization
    // Expected format: projects/{bidId}/{filename}
    const parts = pathname.split('/');
    const uploadType = parts[0];
    const resourceId = parts[1];

    if (uploadType === 'projects' && resourceId) {
      const [bid] = await db
        .select()
        .from(bids)
        .where(
    ...
    const result = await uploadFile(buffer, pathname, { contentType: 'application/pdf' });

    console.log(`[multipart] Upload complete: ${result.url}`);
