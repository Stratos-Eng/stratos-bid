...
export function ProjectViewer({
  ...
  const docInfoRef = useRef<{ id: string; pageCount?: number } | null>(null)

  // Fetch *light* doc info ONCE per document (avoid downloading/parsing PDFs on the server)
  useEffect(() => {
    ...
    const fetchInfo = async () => {
      try {
        const res = await fetch(`/api/documents/${documentId}/info?light=1`)
        if (res.ok) {
          const data = await res.json()
          docInfoRef.current = { id: documentId, pageCount: data.pageCount || 1 }
  ...
  // Page size is derived from PDF.js viewport as pages render; default is fine until then.
