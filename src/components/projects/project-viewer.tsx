...
export function ProjectViewer({
  ...
  // Cache document info to avoid re-fetching on every page change
  const docInfoRef = useRef<{ id: string; pages: Array<{ width?: number; height?: number }> } | null>(null)

  // Fetch page info ONCE per document (not per page change)
  useEffect(() => {
    if (!documentId) return

    // Skip if already cached for this document
    if (docInfoRef.current?.id === documentId) return
    ...
    const fetchInfo = async () => {
      try {
        ...
        if (res.ok) {
          ...
          docInfoRef.current = { id: documentId, pages: data.pages || [] }
    ...
    fetchInfo()
  }, [documentId]) // Only re-fetch when document changes, NOT page

  // Update page dimensions from cache when page changes
  useEffect(() => {
    if (!docInfoRef.current || docInfoRef.current.id !== documentId) return
    const pg = docInfoRef.current.pages[pageNumber - 1]
    if (pg) {
      setPageSize({ width: pg.width || 612, height: pg.height || 792 })
    }
